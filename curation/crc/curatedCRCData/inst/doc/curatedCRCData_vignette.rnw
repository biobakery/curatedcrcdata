...
\section{curatedOvarianData: Clinically Annotated Data for the Colorectal Cancer
Transcriptome}
...
needs the overall survival information as \texttt{Surv} objects, which the
\texttt{createEsetList.R} function already added in the \texttt{phenoData}
slots of the \texttt{ExpressionSets}, accessible at the \texttt{y} label.  The
...
batch effect. We will use the ComBat \citep{Johnson:2007} method, implemented
for example in the SVA Bioconductor package \citep{sva}.  To combine two ExpressionSet
objects, we can use the \texttt{combine()} function. This function will fail
