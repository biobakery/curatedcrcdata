...
celfile.dir <- "../../../DATA/TCGA-RNASeqV2-READ/RAW"

##map between cel files and patient barcodes
celfiles.map <- read.delim("../uncurated/unc.edu_READ.IlluminaHiSeq_RNASeqV2.1.5.0.sdrf.txt",as.is=TRUE)
celfiles.map <- celfiles.map[,match(c("Comment..TCGA.Barcode.", "Extract.Name"),colnames(celfiles.map))]
celfiles.map$alt_sample_name <- sub("-[0-9]{2}[A-Z]-[0-9]{2}[A-Z]-[A-Z][0-9]{3}-[0-9]{2}","",celfiles.map[,1])
...
##start the curation
##--------------------
source("TCGA/TCGA_curation_all_platforms_READ.r")

write.table(curated, row.names=FALSE, file="../curated/TCGA-RNASeqV2-READ_curated_pdata.txt",sep="\t")
