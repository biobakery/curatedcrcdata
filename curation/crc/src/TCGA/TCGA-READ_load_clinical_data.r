original <- read.delim("../uncurated/clinical_patient_read.txt",as.is=TRUE,row.names=1,na.strings=c("[Not Available]", "[Not Applicable]", "[Pending]"),head=T)

##follow-up data:
follow.up <- read.delim("../uncurated/clinical_follow_up_v1.0_read.txt",as.is=TRUE,row.names=1,na.strings=c("[Not Available]", "[Not Applicable]", "[Pending]"))

##stromal data:
clinical.slide <-
  read.delim("../uncurated/biospecimen_aliquot_read.txt", as.is=TRUE,
...
##treatment data:
clinical.drug <-
  read.delim("../uncurated/clinical_drug_read.txt", as.is=TRUE,
             na.strings=c("[Not Available]", "[Not Reported]", "[Not Applicable]", "[Pending]"))
...
##batch
clinical.batch<-read.csv("../uncurated/read_batch.csv",header=T)
